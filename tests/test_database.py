...
from smartmyodoo.core.models import Proposal, TokenUsage, AuditLog

@pytest.fixture(scope="module", autouse=True)
def setup_test_db():
    # Zabezpieczamy, że testy pracują na bazie testowej
    assert "test" in DB_PATH or DB_PATH == "sqlite:///:memory:" or "test" in os.environ.get("DATABASE_URL", ""), "Tests should run on a test database!"
...
def test_crud_proposals(db_session):
    ...
    fetched_updated = db_session.query(Proposal).filter(Proposal.id == new_proposal.id).first()
