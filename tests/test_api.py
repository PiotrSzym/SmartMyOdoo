...
def test_api_crud_operations(client):
    ...
    # 1. CREATE
    res = client.post("/api/secrets/TEST_CRUD_API", json={"password": "api_test_password"}, headers=headers)
    assert res.status_code == 200
...
def test_api_change_pin_non_admin(client):
    res = client.post("/api/change-pin", json={"new_pin": "2222"}, headers={"Authorization": "Bearer 1111"})
    assert res.status_code == 403
    assert "Admin role required" in res.json()["detail"]

def test_api_change_pin_admin(client):
    res = client.post("/api/change-pin", json={"new_pin": "2222"}, headers={"Authorization": "Bearer master"})
    assert res.status_code == 200
    assert res.json()["success"] is True
