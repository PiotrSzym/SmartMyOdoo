...
Powiązane narzędzia: `scripts/odoo_sh_logs.py` (pull logów Odoo.sh przez SSH) · `scripts/gh_flow.py` (bezpieczny GitHub flow: `status/feature/pr/promote/push/install-hook` z twardą blokadą push na prod) · `scripts/bump_manifest.py` (bump wersji `__manifest__.py` wg SemVer domu: major/minor/patch). Logi/diagnostyka: skill `odoo-sh-logs`.
...
## Źródła
**Pełny hub źródeł (docs + fora + SO + błędy):** `docs/ODOO_ZRODLA.md`. Indeks Odoo.sh (19.0): https://www.odoo.com/documentation/19.0/administration/odoo_sh.html
