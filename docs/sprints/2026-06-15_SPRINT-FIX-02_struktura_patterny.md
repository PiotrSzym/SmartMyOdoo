...
|---|---------|-------|------------------------------|--------|
| S3.1 | Rozbić `api.py` na routery domenowe. ✅ **S3.1a** (auth, secrets) — [SPRINT-FIX-02-S3.1](2026-06-16_SPRINT-FIX-02-S3.1_api_routers.md). ⏳ **S3.1b** (chat — wymaga warstwy serwisowej, by router nie odtworzył cyklu globali) | `api.py` → `api_routers/{auth,secrets}.py` (+chat w S3.1b) | ✅ `tests/test_api.py`/`test_security_s13`/`test_api_wiring` bez zmian asercji; suita 219 passed | 🏗️ S3.1a ✅ / S3.1b ⬜ |
