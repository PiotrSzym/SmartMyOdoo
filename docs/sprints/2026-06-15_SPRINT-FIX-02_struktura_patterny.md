...
| S3.2 | Ekstrakcja helperów z `execute`/`execute_stream`: `_check_red_flags`, `_resolve_tools`, `_build_messages`, `_run_tool_call`, `_anon/_deanon` wspólne | `swarm/executor.py` | RED: test, że obie ścieżki dzielą tę samą funkcję polityki (np. red flag wykryty identycznie w `execute` i `execute_stream`); GREEN po ekstrakcji | ⬜ |
