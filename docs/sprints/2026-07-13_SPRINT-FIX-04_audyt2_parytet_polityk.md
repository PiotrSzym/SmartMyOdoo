...
- **Commity:** `f8016ba` (FIX-04 A-1..A-4) + `cba1b97` (hardening S-1/S-2 z /sec) na gałęzi `fix/audit2-a1-a4`.
- **Bramki:** /audyt ✅ APPROVE (A−, S3.2 przywrócony). /sec ⚠️→domknięte: wykrył **S-1** (rezydualny `str(e)` w `execute_stream` `executor.py:756` — ten sam wektor co A-4, bo `chat.py` forwarduje chunki verbatim) i **S-2** (błąd narzędzia bez `_anon` przed LLM, `executor.py:496`); oba naprawione 1-linijkowo + test dowodowy `test_stream_llm_error_does_not_leak_exception_content`. /qa: agent zablokował się na mechanizmie Monitor (NIE błąd testów — pokazywał same zielone); regresję wykonano ręcznie.
- **Evidence (po hardeningu):** podzbiór zmienionych ścieżek (fix04+write-02/03+apply+err-01) = **41 passed / 0 failed**; pełna suita przed S-1/S-2 = 456 passed / 0 failed; ruff clean.
- **TODO przed PR:** gf-review (finalna bramka). Rozważyć dopisanie windowsowego pinu litellm do repo (`constraints-win.txt`; dziś tylko w scratchpad). Pre-existing długi do backlogu: F2 (`_auth_limiter` → `api_deps`), F3 (God Node SkillExecutor 103 edges → /arch), S-2-pokrewne PII w err-path (częściowo domknięte).
