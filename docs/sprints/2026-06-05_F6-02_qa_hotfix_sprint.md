...
| # | Zadanie | DoD | Status |
|---|---------|-----|--------|
| 1.1 | XML: Wskaźnik ładowania | Pole `input` otrzymuje blokadę `t-att-disabled="state.isLoading"`. Dodano mały tekst "Agent myśli..." jeśli isLoading to prawda. | [x] |
| 1.2 | XML: Usunięcie martwych kodów | Usunięto nieużywany i niepotrzebny `t-ref="chatInput"`. | [x] |
| 1.3 | JS: Inicjalizacja Stanu | Dodano pole `isLoading: false` w konstruktorze/setup. | [x] |
| 1.4 | JS: Refactor na `web.rpc` | Usunięto `fetch`. Zaimplementowano `rpc.query({ route: '/smart_chat/send', params: {...} })`. | [x] |
| 1.5 | JS: Blokada Promise | Zmiana `state.isLoading` na `true` przed wysyłką i na `false` w bloku `finally`. | [x] |
| 1.6 | **BRAMKA:** Weryfikacja UI | ✅ Zablokowany interfejs podczas requestu i poprawne rozwiązanie Promise. | [x] |
...
| # | Zadanie | DoD | Status |
|---|---------|-----|--------|
| 2.1 | Zmiana z `list` na `list[Any]` | W pliku `models.py` dla schematu `ChatProposalData` naprawiono linterowy warning Mypy. Zaimportowano `Any`. | [x] |
| 2.2 | **BRAMKA:** Lint Check | ✅ Komenda `ruff check` nie wyrzuca błędów. | [x] |
