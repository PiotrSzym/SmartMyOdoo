...
def get_vault_key_from_pin(pin: str, exit_on_fail: bool = True) -> bytes:
    ...
    with open(PIN_SALT_FILE, "rb") as f: salt = f.read()
    f_pin = Fernet(derive_key(pin, salt))
    with open(PIN_KEY_FILE, "rb") as f: encrypted_vk = f.read()
    try:
...
def get_vault_key_from_master(master_pwd: str, exit_on_fail: bool = True) -> bytes:
    if not os.path.exists(MASTER_SALT_FILE):
        if exit_on_fail:
            ...
            sys.exit(1)
        raise ValueError("Not initialized")
    with open(MASTER_SALT_FILE, "rb") as f: salt = f.read()
    f_master = Fernet(derive_key(master_pwd, salt))
    with open(MASTER_KEY_FILE, "rb") as f: encrypted_vk = f.read()
    try:
...
def init_vault_core(pin: str, master: str) -> None:
    ...
    with open(PIN_SALT_FILE, "wb") as f: f.write(pin_salt)
    with open(PIN_KEY_FILE, "wb") as f: f.write(Fernet(derive_key(pin, pin_salt)).encrypt(vk))
    
    master_salt = os.urandom(16)
    with open(MASTER_SALT_FILE, "wb") as f: f.write(master_salt)
    with open(MASTER_KEY_FILE, "wb") as f: f.write(Fernet(derive_key(master, master_salt)).encrypt(vk))
...
def list_secrets() -> None:
    data = get_secrets() # get_secrets asks for pin if vk is None and cleans up deleted items
    count = 0
...
def copy_secret(key_name: str) -> None:
    ...
    if key_name not in data or (isinstance(data[key_name], dict) and "deleted_at" in data[key_name]):
        print(f"Blad: Klucz '{key_name}' nie istnieje w skarbcu.")
        sys.exit(1)
        
    pyperclip.copy(data[key_name].get("password", ""))
    print(f"Wartosc klucza '{key_name}' skopiowana bezpiecznie do schowka! (Wcisnij Ctrl+V)")
...
def run_wrapped_command(cmd_args: List[str]) -> None:
    ...
    for k, obj in data.items():
        if isinstance(obj, dict):
            if "deleted_at" in obj: continue
            if obj.get("password"): env[f"{k}_PASSWORD"] = str(obj["password"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SmartMyVault - Local Zero-Trust Secret Manager")
    subparsers = parser.add_subparsers(dest="command", help="Dostepne komendy")
    
    subparsers.add_parser("init", help="Inicjalizacja nowego skarbca (PIN + Master Password)")
    ...
    copy_parser = subparsers.add_parser("copy", help="Skopiuj wartosc sekretu do schowka")
    ...
    run_parser = subparsers.add_parser("run", help="Uruchom komende ze wstrzyknietymi zmiennymi ze skarbca")
    run_parser.add_argument("cmd", nargs=argparse.REMAINDER, help="Komenda do uruchomienia")
