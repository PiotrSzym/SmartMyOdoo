...
class OdooClient:
    ...
    def __init__(self, workspace_id: str = "default"):
        ...
        prefix = f"PROJECT_HUB_{workspace_id.upper()}_ODOO" if workspace_id != "default" else "ODOO"
        
        self.url = os.getenv(f"{prefix}_URL") or os.getenv("ODOO_URL")
        self.db = os.getenv(f"{prefix}_DB") or os.getenv("ODOO_DB")
        self.username = os.getenv(f"{prefix}_USERNAME") or os.getenv("ODOO_USERNAME") or os.getenv(f"{prefix}_LOGIN") or os.getenv("ODOO_LOGIN")
    ...
    async def search_read_async(self, model: str, domain: list, fields: list = None, limit: int = 10):
