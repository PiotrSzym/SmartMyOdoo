...
skill = SkillConfig(
    ...
    system_prompt=(
        ...
        "6) Reguła bywa ZARCHIWIZOWANA (active=false) — czytaj base.automation z "
        "context={'active_test': False}, inaczej 'znika'; reaktywacja = write active=True.\n"
        "7) Zmiana na ISTNIEJĄCY adres (ma już ir.mail_server) nie wymaga hasła — tylko "
        "wskazujesz serwer. NOWY adres wymaga serwera z pasującym from_filter, a ten "
        "uwierzytelnienia: skrzynka M365 + app password (nowy serwer) ALBO Send-As dla "
        "istniejącego konta (dopisz adres do jego from_filter). Sprawdź brak serwera: "
        "search(ir.mail_server, [('from_filter','ilike','<adres>')]).\n"
        "8) PRZED podpięciem reguły odpal test logowania SMTP: "
        "ir.mail_server.test_smtp_connection([id]) — sukces=notyfikacja, błąd=Fault.\n"
        "9) Test bezpieczny MUSI mieć realny res_id: mail.mail dziedziczy mail.message, "
        "więc create z model='project.task' i res_id=0 crashuje na _get_reply_to "
        "(KeyError: False). Utwórz TYMCZASOWY project.task, mail state='cancel', sprawdź "
        "email_from+mail_server_id, potem unlink maila i zadania; przy porażce rollback "
        "active=False. Nie celuj w realne zadanie (zaśmiecasz jego chatter).\n"
